...
export const setUserInfo = createAction<UserAuth | null>(
  'user/setUserInfo'
);
...
>('user/checkAuth', async (_arg, { dispatch, extra: api }) => {
  try {
    ...
    const { data, status } = await api.get<UserAuth>('/login', { headers });
...
>('user/login', async ({ email, password }, { dispatch, extra: api }) => {
  try {
    const { data, status } = await api.post<UserAuth>('/login', {
      email,
      password,
    });
